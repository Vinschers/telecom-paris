...
viewimage(rotation(im,45,ech=0))
viewimage(rotation(im,45,ech=1))

# %%

imr = im
for _ in range(8):
    imr = rotation(imr,45,ech=0)
viewimage(imr)

imr = im
for _ in range(8):
    imr = rotation(imr,45,ech=1)
viewimage(imr)

# %%
im=skio.imread('images/lena.tif')
iml = filtre_lineaire(im, get_cst_ker(5))
viewimage(rotation(im,45, 0.5,ech=1, clip=False))
viewimage(rotation(iml,45, 0.5,ech=1, clip=False))
viewimage(filterlow(rotation(iml,45, 0.5,ech=1, clip=False)))
# %%
for i in range(10):
    print(i, get_gau_ker(i).size)
    
# %%

im = skio.imread("images/pyramide.tif")
print(var_image(im, 0, 0, 30, 30))
imn = noise(im, 10)
print(var_image(imn, 0, 0, 30, 30))
print(var_image(filtre_lineaire(imn, get_cst_ker(5)), 0, 0, 30, 30))
# %%
...
im=skio.imread('images/pyrabruit.tif')
viewimage(im)
viewimage(filtre_lineaire(im, get_cst_ker(3)))
viewimage(median_filter(im, r=3))
# %%
...
imf = filtre_lineaire(im, mask)
imf = noise(imf, 10)
inv = filtre_inverse(imf, mask)
...
im_median = median_filter(im, 2, 4)
viewimage(im_median)

print(var_image(im_median, 0, 0, 30, 30))

for i in range(1, 17):
    mask = get_cst_ker(i)
    im_cst = filtre_lineaire(im, mask)
    # print(i, var_image(im_cst, 0, 0, 30, 30))
mask = get_cst_ker(2)
im_cst = filtre_lineaire(im, mask)
print(2, var_image(im_cst, 0, 0, 30, 30))
...
def wiener_mod(im,K,lamb=0):
    ...
    #fonction de mutilplication
    sigma_s = (g)**2
    sigma_b = np.where(sigma_s != 0, im.var()*len(im)/sigma_s, 0)
...
viewimage(wiener_mod(im,get_gau_ker(2)))
